          ...
          bool expected_result)
{
  ...
  ast::ConfigAnalyser config_analyser(driver.ctx, bpftrace.config(), out);
...
TEST(config_analyser, config_setting)
{
  ...
  EXPECT_NE(bpftrace->config().get(ConfigKeyInt::max_map_keys), 9);
  test(*bpftrace, "config = { BPFTRACE_MAX_MAP_KEYS=9 } BEGIN { }");
  EXPECT_EQ(bpftrace->config().get(ConfigKeyInt::max_map_keys), 9);

  EXPECT_NE(bpftrace->config().get(ConfigKeyStackMode::default_),
            StackMode::perf);
  test(*bpftrace, "config = { stack_mode=perf } BEGIN { }");
  EXPECT_EQ(bpftrace->config().get(ConfigKeyStackMode::default_),
            StackMode::perf);

  EXPECT_NE(bpftrace->config().get(ConfigKeyUserSymbolCacheType::default_),
            UserSymbolCacheType::per_program);
  EXPECT_NE(bpftrace->config().get(ConfigKeyInt::log_size), 150);
  test(*bpftrace,
       "config = { BPFTRACE_CACHE_USER_SYMBOLS=\"PER_PROGRAM\"; log_size=150 "
       "} BEGIN { }");
  EXPECT_EQ(bpftrace->config().get(ConfigKeyUserSymbolCacheType::default_),
            UserSymbolCacheType::per_program);
  EXPECT_EQ(bpftrace->config().get(ConfigKeyInt::log_size), 150);

  // When liblldb is present, the default config for symbol_source is "dwarf",
  // otherwise the default is "symbol_table".
#ifdef HAVE_LIBLLDB
  EXPECT_EQ(bpftrace->config().get(ConfigKeySymbolSource::default_),
