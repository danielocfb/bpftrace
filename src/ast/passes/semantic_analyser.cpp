...
void SemanticAnalyser::visit(String &string)
{
  ...
  auto str_len = bpftrace_.config().get(ConfigKeyInt::max_strlen);
...
void SemanticAnalyser::visit(Builtin &builtin)
{
  if (builtin.ident == "ctx") {
    ...
    builtin.type = CreateStack(true,
                               StackType{ .mode = bpftrace_.config().get(
    ...
    for (auto *attach_point : probe->attach_points) {
      ...
      if (type == ProbeType::uprobe &&
          bpftrace_.config().get(ConfigKeyBool::probe_inline))
        LOG(ERROR, builtin.loc, err_)
...
void SemanticAnalyser::visit(Call &call)
{
  ...
  if (call.func == "hist") {
    ...
    if (check_varargs(call, 1, 2)) {
      ...
      auto strlen = bpftrace_.config().get(ConfigKeyInt::max_strlen);
    ...
    const uint64_t max_strlen = bpftrace_.config().get(ConfigKeyInt::max_strlen);
    if (max_strlen >
    ...
    if (check_varargs(call, 1, 2)) {
      ...
      auto call_type_size = bpftrace_.config().get(ConfigKeyInt::max_strlen);
      if (call.vargs.size() == 2) {
        if (check_arg(call, Type::integer, 1, true)) {
...
void SemanticAnalyser::check_stack_call(Call &call, bool kernel)
{
  ...
  stack_type.mode = bpftrace_.config().get(ConfigKeyStackMode::default_);
...
void SemanticAnalyser::visit(AttachPoint &ap)
{
  if (ap.provider == "kprobe" || ap.provider == "kretprobe") {
    ...
    if (is_final_pass()) {
      ...
      if (bpftrace_.config().get(ConfigKeyMissingProbes::default_) !=
